...
struct Image {
  ...
  void copy(Image& out) {
    ...
    CHECK_HIP(hipMalloc(&out.pixels, width*height*sizeof(Pixel)));
...
__global__ void applySobelFilter_gpu(Pixel* d_input, Pixel* d_output, int width, int height) 
{
  ...
  int gx, gy;
  if (x > 0 && x < width - 1 && y > 0 && y < height - 1) {
    gx = -1 * ( (d_input[(y - 1) * width + (x - 1)].r + d_input[(y-1)*width + (x-1)].g  + d_input[(y-1)*width + (x-1)].b ) /3 )  +
          -2 * ( (d_input[y*width + (x-1)].r + d_input[y*width + (x-1)].g + d_input[y*width + (x-1)].b) / 3) +
          -1 * ( (d_input[(y+1)*width + (x-1)].r + d_input[(y+1)*width + (x-1)].g + d_input[(y+1)*width + (x-1)].b) /3) +
          1 * ( ( d_input[(y-1)*width + (x+1)].r + d_input[(y-1)*width + (x+1)].g + d_input[(y-1)*width + (x+1)].b) /3) +
          2 * ( (d_input[y*width + (x+1)].r + d_input[y*width + (x+1)].g + d_input[y*width + (x+1)].b ) /3 ) +
          1 * ( (d_input[(y+1)*width + (x+1)].r + d_input[(y+1)*width + (x+1)].g + d_input[(y+1)*width + (x+1)].b) /3);

    gy =  -1 * ((d_input[(y-1)*width + (x-1)].r + d_input[(y-1)*width + (x-1)].g + d_input[(y-1)*width + (x-1)].b) /3) +
          -2 * ((d_input[(y-1)*width + x].r + d_input[(y-1)*width + x].g + d_input[(y-1)*width + x].b) /3) +
          -1 * ((d_input[(y-1)*width + (x+1)].r + d_input[(y-1)*width + (x+1)].g + d_input[(y-1)*width + (x+1)].b) /3) +
          1 * ((d_input[(y+1)*width + (x-1)].r + d_input[(y+1)*width + (x-1)].g + d_input[(y+1)*width + (x-1)].b)/3) +
          2 * ((d_input[(y+1)*width + x].r + d_input[(y+1)*width + x].g + d_input[(y+1)*width + x].b) /3) +
          1 * ((d_input[(y+1)*width + (x+1)].r + d_input[(y+1)*width + (x+1)].g + d_input[(y+1)*width + (x+1)].b) /3) ;
    ...
    magnitude = min(max(magnitude, 0), 255);
    if ( x == 1 && y == 1) {
      printf("gx=%d, gy=%d, magnitude=%d\n", gx, gy, magnitude);
    }
    d_output[y*width + x] = {static_cast<unsigned char>(magnitude),
      static_cast<unsigned char>(magnitude),
      static_cast<unsigned char>(magnitude)};
  }
...
void applySobelFilter_hip(Image& input, Image& output) {
  ...
  CHECK_HIP(hipMemcpyAsync(d_input, input.pixels, size, hipMemcpyHostToDevice));
  ...
  applySobelFilter_gpu<<<dimGrid, dimBlock>>>(d_input, d_output, width, height);
  CHECK_HIP(hipDeviceSynchronize());
  // hipMemcpy(output.pixels, d_output, size, hipMemcpyDeviceToHost);
  CHECK_HIP(hipMemcpyAsync(output.pixels, d_output, size, hipMemcpyDeviceToHost));
  CHECK_HIP(hipDeviceSynchronize());
  hipFree(d_input);
  hipFree(d_output);
}
...
int main(int argc, char **argv) {
  ...
  //{
  applySobelFilter_hip(inputImage, outputImage_hip);
  //}
